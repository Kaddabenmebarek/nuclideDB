...
--NEW--
PROMPT Creating TABLE nuclide_attached
CREATE TABLE nuclide_attached (
    nuclide_attached_id		NUMBER(8,0)		NOT NULL
        CONSTRAINT pk_nuclide_attached
        PRIMARY KEY
        USING INDEX TABLESPACE &tab_space_index
        STORAGE (INITIAL 20k),
	...
	user_id					VARCHAR2(8)		
        CONSTRAINT fk_nuclid_attached_user
        REFERENCES nuclide_user(user_id),
	nuclide_bottle_id		NUMBER(8,0)		
		CONSTRAINT fk_nuclide_attached_bottle
		REFERENCES nuclide_bottle(nuclide_bottle_id)
	)
	TABLESPACE &tab_space_table
	STORAGE (INITIAL 800k);
...
GRANT SELECT ON nuclide_attached TO nuclide_reader;